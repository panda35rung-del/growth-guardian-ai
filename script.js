...
// คำนวณอายุจากวันเกิด (ISO) → {years, months, days, text}
function calculateAge(birthISO) {
  ...
  if (m < 0) { y--; m += 12; }
  return { years: y, months: m, days: d, text: `${y} ปี ${m} เดือน ${d} วัน` };
}

// หาช่วงอายุประเมิน → "2"/"3"/"4"/"5" หรือ null ถ้านอกช่วง 2.0–5.11
...
// คำนวณอายุ + เลือกช่วงอายุ เมื่อกรอกวันเกิด
function onBirthDateChange() {
  ...
  if (!age) { ageTextEl.value = ""; warn.classList.remove("show"); return; }
...
async function submitStep1() {
  ...
  if (!fullName) return toast("กรุณากรอกชื่อ - นามสกุล", "warn");
  if (!birthDate) return toast("กรุณาเลือกวันเกิด", "warn");
  const age = calculateAge(birthDate);
  const group = getAgeGroup(age);
  if (!group) return toast("อายุอยู่นอกช่วงประเมิน (2.0 – 5.11 ปี)", "warn");
  if (!weight || !height) return toast("กรุณากรอกน้ำหนักและส่วนสูง", "warn");
  ...
  const studentId = isEditing ? currentStudent.studentId : ("STD" + Date.now().toString().slice(-6));
  ...
  if (isEditing) {
    ...
    toast(DB.mode === "cloud" ? "บันทึกนักเรียนขึ้น Cloud สำเร็จ" : "บันทึกนักเรียนในเครื่องแล้ว", "ok");
  }
...
async function init() {
  ...
  document.getElementById("btnToStep2").onclick = submitStep1;
